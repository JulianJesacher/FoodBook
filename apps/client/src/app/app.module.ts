...
import {AppConfigurationService} from "./services/configurationService/app-configuration.service";

function initializeUser(configService: AppConfigurationService, userDataService: UserDataService): () => Observable<any> {
  return () => new Observable((subscriber) => {
    configService.init().subscribe(() => {
      userDataService['initialize']().pipe(catchError((err) => of())).subscribe(() => {
        subscriber.complete();
      });
    })
  });
...
@NgModule({
  ...
  providers: [
    ...
    {provide: HTTP_INTERCEPTORS, useClass: TokenInterceptor, multi: true},
    {
      ...
      deps: [AppConfigurationService, UserDataService, AuthService, HttpClient],
      multi: true
    },
