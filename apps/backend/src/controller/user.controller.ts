...
export class UserController {
    public static async signup(req: Request, res: Response): Promise<void> {
        ...
        if (UserHelper.checkAvailability(req, res, userData)) {
            ...
            res.status(409).send({ message: 'Username or email already taken' });
        }
    }
    ...
    public static async usernameExists(req: Request, res: Response): Promise<void> {
        const exists =
            (await User.createQueryBuilder().where('username = :username', { username: req.params.username }).getOne()) !== undefined;
